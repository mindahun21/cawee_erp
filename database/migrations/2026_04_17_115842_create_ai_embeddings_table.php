...
return new class extends Migration
{
    ...
    {
        // Skip if vector database is not available (e.g., cPanel deployment without PostgreSQL)
        if (!VectorDatabaseDetector::isAvailable()) {
            Log::info('Skipping ai_embeddings migration: vector database unavailable', [
                'hint' => 'This is expected on cPanel or hosting without PostgreSQL/pgvector'
            ]);
            return;
        }
        ...
        try {
            ...
            Log::info('ai_embeddings migration completed successfully');
        } catch (\Exception $e) {
            // Log warning but don't throw - allow migration to succeed
            Log::warning('ai_embeddings migration failed', [
                'error' => $e->getMessage(),
                'hint' => 'This is expected if vector database is unavailable or misconfigured'
            ]);
        }
    }
    ...
    {
        // Only attempt to drop if vector database is available
        if (VectorDatabaseDetector::isAvailable()) {

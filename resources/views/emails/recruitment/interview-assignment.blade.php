...
<style>
  ...
  .strip{height:5px;background:#2563eb;}
  ...
  .status-badge{background:#dbeafe;color:#1e40af;border:1px solid #bfdbfe;padding:6px 14px;border-radius:20px;font-size:12px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;display:inline-block;margin-bottom:20px;}
...
<div class="wrap">
  ...
  <div class="strip"></div>
  <div class="body">
    <div class="status-badge">📅 Assignment</div>
    <p>Hello <strong>{{ $notifiable->name }}</strong>,</p>
    <p>You have been assigned as an <strong>{{ ucfirst($role) }}</strong> for the upcoming interview schedule: <strong>{{ $schedule->name }}</strong>.</p>
      ...
      <div class="detail-row">
        ...
        <span class="detail-value">
          @if(filter_var($schedule->location, FILTER_VALIDATE_URL))
            <a href="{{ $schedule->location }}" style="color:#003366;text-decoration:none;">Join Meeting</a>
          @else
            {{ $schedule->location }}
          @endif
        </span>
    ...
    <p style="margin-top:24px;">Please review the candidate details and evaluation forms in the system before the session begins.</p>
    
    @if(!empty($viewUrl))
    <div style="text-align:center;margin:32px 0 16px;">
      <a href="{{ $viewUrl }}" style="display:inline-block;background:#2563eb;color:#fff;font-weight:700;font-size:15px;padding:14px 36px;border-radius:8px;text-decoration:none;letter-spacing:.02em;">View Schedule Details →</a>
    </div>
    @endif
  </div>
  <div class="footer">

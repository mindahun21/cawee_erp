...
<div class="wrap">
  <div class="header">
    ...
    <h1>Recruitment Campaign Approved</h1>
  </div>
  <div class="strip"></div>
  <div class="body">
    <div class="congrats">✅</div>
    <div class="status-badge">✓ Approved & Active</div>
    <p>Your recruitment campaign has been <strong>approved</strong> and is now <strong>active</strong>. Candidates can start applying.</p>
    ...
    <div class="detail-box">
      ...
      <div class="detail-row">
        ...
        <span class="detail-value">{{ $campaign->jobPosition->title ?? '—' }}</span>
      </div>
      <div class="detail-row">
        <span class="detail-label">Vacancies:</span>
        <span class="detail-value">{{ $campaign->vacancies_needed }}</span>
      </div>
      <div class="detail-row">
        <span class="detail-label">Employment Type:</span>
        <span class="detail-value">{{ ucwords(str_replace('_', ' ', $campaign->employment_type)) }}</span>
      </div>
    </div>
    ...
    <div class="next-steps">
      <h3>Next Steps</h3>
      <ul>
        <li>The campaign is now live and visible to candidates on the recruitment portal.</li>
        <li>Monitor incoming applications under <strong>Recruitment → Applications</strong>.</li>
      </ul>
    </div>

    @if(!empty($viewUrl))
    <div style="text-align:center;margin:24px 0 16px;">
      <a href="{{ $viewUrl }}" style="display:inline-block;background:#16a34a;color:#fff;font-weight:700;font-size:15px;padding:14px 36px;border-radius:8px;text-decoration:none;letter-spacing:.02em;">View Campaign →</a>
    </div>
    @endif
  </div>
  <div class="footer">

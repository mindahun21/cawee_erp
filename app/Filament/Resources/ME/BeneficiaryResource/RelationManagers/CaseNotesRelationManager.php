    ...
    public function form(Schema $schema): Schema
    {
        return $schema->components([
            ...
            Textarea::make('content')
                ->label('Note')->required()->rows(5),

            DatePicker::make('follow_up_date')
                ->label('Follow-Up Date')
                ->native(false)->minDate(now()),
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('content')
                    ->label('Note')->limit(80)->wrap(),

                TextColumn::make('author.name')
                    ->label('Case Worker')->placeholder('—'),

                TextColumn::make('follow_up_date')
                    ->label('Follow-Up')->date()->placeholder('—')
                    ->color(fn (MeCaseNote $record): string => $record->isOverdue() ? 'danger' : 'gray'),

                IconColumn::make('is_confidential')
                    ->label('Confidential')->boolean(),

                TextColumn::make('created_at')
                    ->label('Added')->since()->sortable(),
            ])

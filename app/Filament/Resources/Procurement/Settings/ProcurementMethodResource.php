...
class ProcurementMethodResource extends Resource
{
    ...
    protected static ?string $navigationLabel = 'Procurement Methods';
            ...
            Section::make('Method Details')
                ...
                ->schema([
                    TextInput::make('name')
                        ->required()
                        ->maxLength(100)
                        ->placeholder('e.g., Open Tender, Direct Procurement')
                        ->unique(ProcurementMethod::class, 'name', ignoreRecord: true),

                    Toggle::make('is_active')
                        ->label('Active')
                        ->default(true)
                        ->helperText('Inactive methods will not appear in dropdowns for new documents.'),
                ]),
        ]);
    }
